...
#    - repeated search: 
#        - apply gradient descent from different optimuming points.
#    - I think there are 5 maxima. But I'm not sure :-P
# prepare to cry:
#    - find the optimum for all four parameters
#    - define your own algorithm!
def move(position, first, second):
    move = utility(position[1][0] + first, position[1][1] + second)
    return move

def find_highest(possible_moves):
    highest = possible_moves[0]
    for i in range(len(possible_moves)):
        if utility(possible_moves[i][1][0], possible_moves[i][1][1]) > utility(highest[1][0], highest[1][1]):
            highest = i
    return highest
...
def find_next_step(position):
    next_step = (1, 1)
    possible_moves = [[move(position, 1, 1), (1, 1)], [move(position, 1, -1), (1, -1)], [move(position, -1, 1), (-1, 1)], [move(position, -1, -1), (-1,-1)]]

    next_step = find_highest(possible_moves)
...
def find_maximum():
    optimum = [random.randint(1, 101), random.randint(1, 101)]
    value = utility(optimum[0], optimum[1])

    start = (value, optimum)
    next = find_next_step(start)
